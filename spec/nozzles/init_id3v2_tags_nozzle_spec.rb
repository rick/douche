...
describe InitId3v2TagsNozzle do
  ...
  describe 'once initialized' do
    ...
    describe 'when determining if id3v2 tags are needed for a file' do
      ...
      it 'should return :create when there are no id3 tags for the file' do
        stub(@info).hastag1? { false }
        stub(@info).hastag2? { false }
        @nozzle.needs_v2_tags?(@file).should == :create
      end
      
      it 'should return :upgrade when there are only id3v1 tags for the file' do
        stub(@info).hastag1? { true }
        stub(@info).hastag2? { false }
        @nozzle.needs_v2_tags?(@file).should == :upgrade
      end
    ...
    it 'should have a means of creating id3 tags' do
      @nozzle.should respond_to(:create_tags)
    end
    
    describe 'when creating id3 tags' do
      it 'should accept a file path' do
        lambda { @nozzle.create_tags(@file) }.should_not raise_error(ArgumentError)
      end
      
      it 'should require a file path' do
        lambda { @nozzle.create_tags }.should raise_error(ArgumentError)
      end
      
      it 'should set empty id3v2 tags for artist, album, genre, and title' do
        pending("figuring out how to test this")
      end
    end
    ...
    describe 'when spraying a file' do
      ...
      describe 'when the id3v2 tags cannot be read' do
        ...
        it 'should not create id3v2 tags' do
          mock(@nozzle).create_tags(@file).never
          @nozzle.spray(@file)
        end
      ...
      describe 'when the file has no id3v2 tags' do
        describe 'but it has id3v1 tags' do
          before :each do
            stub(@nozzle).needs_v2_tags?(@file) { :upgrade }
          end
        ...
        describe 'and it has no id3v1 tags' do
          before :each do
            stub(@nozzle).needs_v2_tags?(@file) { :create }
          end
          ...
          it 'should create id3v2 tags' do
            mock(@nozzle).create_tags(@file)
            @nozzle.spray(@file)
          end
